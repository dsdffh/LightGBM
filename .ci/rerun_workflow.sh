...
runs=$(curl -sL \
  ...
  "${GITHUB_API_URL}/repos/StrikerRUS/LightGBM/actions/workflows/${workflow_id}/runs?event=pull_request&branch=${pr_branch}" | \
  jq '.workflow_runs')
...
if [[ $(echo $runs | jq 'length') -gt 0 ]]; then
  curl -sL \
    ...
    -H "Accept: application/vnd.github.v3+json" \
    -H "Authorization: token $SECRETS_WORKFLOW" \
    "${GITHUB_API_URL}/repos/StrikerRUS/LightGBM/actions/runs/$(echo $runs | jq '.[0].id')/rerun"
fi
